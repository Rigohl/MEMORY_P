...
// Import from library
use memory_p::cli::{Commands, Cli};
use memory_p::cli::{validators, sql_detector, auto_repair};
...
fn main() {
    ...
    let result = match cli.command {
        Commands::Validate { path, check_dead_code, scan_todos, validate_mcp } => {
            handle_validate(&path, check_dead_code, scan_todos, validate_mcp)
        }
        Commands::DetectSql { path, validate_syntax, detect_issues } => {
            handle_detect_sql(&path, validate_syntax, detect_issues)
        }
        Commands::Repair { path, fix_deps, format, fix_clippy, regen_schemas, dry_run } => {
            handle_repair(&path, fix_deps, format, fix_clippy, regen_schemas, dry_run)
        }
        Commands::CiCheck { path } => {
            handle_ci_check(&path)
        }
    };
...
fn print_banner() {
    println!("{}", "╔══════════════════════════════════════════════════╗".bright_blue());
    println!("{}", "║      JAR - Intelligent DevOps CLI v0.1.0        ║".bright_blue());
    println!("{}", "║           For MEMORY_P Project                   ║".bright_blue());
    println!("{}", "╚══════════════════════════════════════════════════╝".bright_blue());
}

fn handle_validate(path: &str, check_dead_code: bool, scan_todos: bool, validate_mcp: bool) -> Result<i32, anyhow::Error> {
...
fn handle_detect_sql(path: &str, validate_syntax: bool, detect_issues: bool) -> Result<i32, anyhow::Error> {
    let report = sql_detector::detect_sql(path, validate_syntax, detect_issues)?;
    report.print();
    
    Ok(if !report.syntax_errors.is_empty() { 1 } else { 0 })
}
...
fn handle_repair(
    ...
    let (fix_deps, format, fix_clippy) = if !fix_deps && !format && !fix_clippy && !regen_schemas {
        println!("{} No specific repair actions specified, enabling: format + fix_deps", "ℹ️".yellow());
        (true, true, false)
    } else {
        (fix_deps, format, fix_clippy)
    };
    
    let report = auto_repair::repair_project(path, fix_deps, format, fix_clippy, regen_schemas, dry_run)?;
    report.print();
...
fn handle_ci_check(path: &str) -> Result<i32, anyhow::Error> {
    ...
    if !workflow_path.exists() {
        println!("\n{} Workflows directory not found at {}", "⚠️".yellow(), path);
