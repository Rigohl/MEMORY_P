...
impl ContextManager {
    ...
    pub async fn get_or_create(&self, agent_id: AgentId) -> Result<SharedContext> {
        ...
        self.agent_index.insert(agent_id, context.context_id.clone());
    ...
    pub async fn delete(&self, context_id: &ContextId) -> Result<()> {
        if let Some((_, context)) = self.contexts.remove(context_id) {
            ...
            Err(MemoryPError::SharedMemoryError(
                format!("Contexto {} no encontrado", context_id)
            ))
        }
    }
...
mod tests {
    ...
    async fn test_update_context() {
        ...
        context.shared_data.insert("key".to_string(), serde_json::json!("value"));
