...
pub fn validate_project(
    ...
    if !cargo_toml.exists() {
        report
            .errors
            .push(format!("Cargo.toml not found at {}", cargo_toml.display()));
        return Ok(report);
    }

    // Parse Cargo.toml
    let cargo_content = fs::read_to_string(&cargo_toml).context("Failed to read Cargo.toml")?;

    // Check for package name
    if !cargo_content.contains("[package]") {
        report
            .errors
            .push("Cargo.toml missing [package] section".to_string());
    }
    ...
    if !src_dir.exists() {
        report
            .errors
            .push(format!("src/ directory not found at {}", src_dir.display()));
    } else {
        ...
        if !has_main && !has_lib {
            report
                .warnings
                .push("Neither main.rs nor lib.rs found in src/".to_string());
        }
    }
...
fn validate_mcp_structure(path: &Path, report: &mut ValidationReport) -> Result<()> {
    ...
    if !has_mcp_api {
        report
            .warnings
            .push("No MCP API implementation found (src/mcp_api.rs or src/mcp/)".to_string());
    }
    ...
    if cargo_toml.exists() {
        ...
        if !content.contains("mcp-sdk") && !content.contains("mcpkit") {
            report
                .mcp_issues
                .push("No MCP SDK dependency found in Cargo.toml".to_string());
        }

        if !content.contains("axum") && !content.contains("actix-web") {
            report
                .mcp_issues
                .push("No web framework found (axum/actix-web)".to_string());
        }
    }
