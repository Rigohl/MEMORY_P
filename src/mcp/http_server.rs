...
// ── Protocol Constants ──────────────────────────────────────────────────
const MCP_PROTOCOL_VERSION: &str = "2024-11-05";  // ← STANDARD SPEC (fixed)
const SERVER_NAME: &str = "MEMORY_P";
const SERVER_VERSION: &str = "3.0.0";
#[allow(dead_code)]
const CAPABILITY_MEMORY_BANK: bool = true;  // ← MEMORY_P extension
...
async fn mcp_post(
    ...
    if req.method != "initialize" {
        if let Some(session_id) = headers.get("mcp-session-id").and_then(|v: &axum::http::HeaderValue| v.to_str().ok()) {
    ...
    let response = match req.method.as_str() {
        ...
        "notifications/initialized" | "initialized" => {
            if let Some(sid) = headers.get("mcp-session-id").and_then(|v: &axum::http::HeaderValue| v.to_str().ok()) {
        ...
        "tools/call" => {
            ...
            let tool_name = params.get("name").and_then(|v: &serde_json::Value| v.as_str()).unwrap_or("");
