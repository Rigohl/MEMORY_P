...
use crate::shared_memory::{SharedMemorySystem, AgentId, MemoryStats};
...
impl SharedMemoryToolHandler {
    ...
    pub async fn update_agent_context(&self, params: UpdateContextParams) -> Result<Value> {
        ...
        self.system.update_context(agent_id, context.clone()).await?;
    ...
    pub async fn sync_contexts(&self, params: SyncContextsParams) -> Result<Value> {
        ...
        let target_agents: Vec<AgentId> = params.target_agents
            .into_iter()
            .map(AgentId::new)
            .collect();
        
        self.system.sync_contexts(source_agent, target_agents.clone()).await?;
